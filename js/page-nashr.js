/* nashr.html sahifasining skripti.
  ...
  Site.initPage({ active:'pubs', render(){
    ...
    if(!p || p.status!=='published'){
      main.innerHTML = `<div class="page-banner"><div class="wrap"><div class="crumb">${Site.crumbHTML('nashrlar.html')}</div><h1>${T('not_found_t')}</h1></div></div>
    ...
    const dlBtn = p.pdf
      ? `<a class="btn dlbtn" href="${Site.safeUrl(p.pdf)}" download><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" class="ico-16"><path d="M12 4v11m0 0 4-4m-4 4-4-4M5 19h14" stroke-linecap="round" stroke-linejoin="round"/></svg>${T('download_pdf')}</a>`
      : `<button class="btn outline dlbtn btn-soon" disabled>${T('soon')}</button>`;
    const meta = [
    ...
    const bylinePrint = p.author
      ? `<div class="print-byline">`
        + `<div class="pb-lab">${Site.esc(T('f_author'))}</div>`
        + `<div class="pb-nm">${Site.esc(p.author)}</div></div>`
      : '';
    ...
    // Non zanjiri nashr QAYSI kichik sahifada turishiga qarab quriladi
    // (Maqolalar / Ma'ruzalar / Tahlillar / Kitoblar). Ilgari oxirgi band
    // nashrning O'Z TOIFASI edi ("Siyosat" kabi) — menyuda bunday bo'lim yo'q.
    const pk = Site.pubKind(p.type);
    main.innerHTML = `${printHead}
      <div class="page-banner tight${showFull?' has-full':''}"><div class="wrap">
        <div class="crumb">${Site.crumbHTML(pk ? pk.page : 'nashrlar.html')}</div>
      ...
      <section class="block"><div class="wrap"><div class="pub-detail">
        ...
        <div>
          ...
          <div class="kicker mb-18">${T('pub_about')}</div>
      ...
      ${related.length?`<section class="block related-sec pt0"><div class="wrap">
        ...
        <div class="pub-grid">${related.map(r=>`
          <a class="pub rv" href="nashr.html?id=${r.id}">
